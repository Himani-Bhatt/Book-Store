...
import { Box, Card as MuiCard, CardContent, CardMedia, Typography, Button, Chip } from '@mui/material';
import "./style.css"
  ...
  const onOrderCancel = (e) => {
    ...
    dispatch(cancelOrder(book.id))
      ...
      .then(() => {
        ...
        const updatedOrders = orders.filter((order)=>order.id !== book.id)
  ...
  return (
    <MuiCard className="card">
      ...
      <CardContent>
        ...
        <Typography variant="h6" color="text.primary" className='card-price'>
          ${book.price}
        </Typography>
        <div className="tag-container">
          {book.tags.map((tag, index) => (
            <Chip key={index} label={tag} sx={{
                backgroundColor: '#000',
                color: '#fff',
                margin: '5px',
            }} />
      ...
      {canCancel && <Button onClick={openModal} variant="contained" sx={{
        backgroundColor: '#000',
        color: '#fff',
        border: 'none',
        padding: '10px 20px',
      ...
      {canOrder && <Button onClick={openOrderModal} variant="contained" sx={{
        backgroundColor: '#000',
        color: '#fff',
        border: 'none',
        padding: '10px 20px',
      ...
      >
        <h2>Cancel Order</h2>
        <button style={{ padding: '16px' }} className='btn-close' onClick={closeModal}>
          X
        </button>
        <form onSubmit={onOrderCancel}>
          ...
          <div style={{ display: 'flex', gap:2 }} className='form-group'>
      ...
      >
        <h2>Create Order</h2>
        <button  style={{ padding: '16px' }}  className='btn-close' onClick={closeOrderModal}>
          X
        </button>
